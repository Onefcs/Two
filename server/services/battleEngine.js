// Pure, DB-free combat simulator. Given two combatants it resolves an entire fight
// tick-by-tick (100ms ticks, capped at 300 = 30s) and returns a full event log the
// client can replay — the server is the sole source of truth for outcome/rewards.
...
function makeCombatant({ id, side, stats, skills }) {
  return {
    ...
    maxHp: stats.hp,
    hp: stats.hp,
    stats, // effective stats (base + upgrades + equipment), pre-buff
    skills: [...skills].sort((a, b) => a.priority - b.priority),
    cooldowns: {}, // skillKey -> lastFiredAt (ms)
    lastBasicAttackAt: -Infinity,
    effects: [], // { stat, kind: 'mult'|'add', value, expiresAt }
    dots: [], // { dmgPerHit, nextTickAt, expiresAt }
  };
}
...
function tickDots(combatant, now, log, tick) {
  for (const dot of combatant.dots) {
    if (now >= dot.nextTickAt) {
      ...
      log.push({
        tick, time: now, actorSide: combatant.side === 'player' ? 'monster' : 'player', targetSide: combatant.side,
...
function actOnce(actor, target, now, rng, log, tick) {
  ...
  if (now - actor.lastBasicAttackAt >= basicCadence) {
    ...
    applySkillEffect(actor, target, { key: 'basic_attack', name: 'Обычная атака', effect_type: 'damage', power_multiplier: 1, extra: {} }, now, rng, log, tick);
    return true;
  }
...
function simulateBattle(player, monster, rngFactory = defaultRng) {
  ...
  let outcome;
  if (p.hp <= 0 && m.hp <= 0) outcome = 'loss'; // simultaneous death favors the monster (hardcore)
...
module.exports = { simulateBattle, makeCombatant, effectiveStats, TICK_MS, MAX_TICKS };
